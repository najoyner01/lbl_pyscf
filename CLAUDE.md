...
| Gap | Where | Notes |
|-----|-------|-------|
| **Spin-orbit ECP (SO-ECP)** | branch `ecp-so`; design in `docs/ecp-so-design.md` | Python scaffold + CUDA kernel written, **needs GPU validation**. `sort_ecp_basis_so`, `get_ecp_so` → `[3,nao,nao]` real, `get_soc_1e` (Pauli assembly); `lib/ecp/ecp_so.cu` (`so_cart` = `type2_cart` + `L^a` transform of `omegaj` + ½) + `ECP_so_cart` entry in `nr_ecp_driver.cu`; `so_ang_matrix.cu` constants; `test_ecp_so.py`. Rebuild `libgecp`, run test on Perlmutter. |
